...
export class DistributerListComponent {
  ...
  getDistributerList() {
    ...
    let payload = {
      "roleId": Number(this.userDetails?.RoleId),
      "parentId": this.selectedManufacture ? Number(this.selectedManufacture?.value) : Number(this.userDetails?.Id)
    }
    this.distributerService.distributerList(payload).subscribe((res: any) => {
      this.isLoading = false
      if (res?.body?.isSuccess == true) {
        this.distributerList = res?.body?.result || []
        console.log("this.distributerList",this.distributerList);
        
        this.pagesize.count = this.distributerList?.length
